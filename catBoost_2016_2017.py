...
def main():
    ...
    trainCls = trainClass()
    train6_df = trainCls.readTrain2016()
    # pd.read_csv('./train_2016_v2.csv', parse_dates=['transactiondate'], low_memory=False)
    ...
    #pd.read_csv('./properties_2017.csv', low_memory=False)
    properties_6 = trainCls.readProp2016()
    #pd.read_csv('./properties_2016.csv', low_memory=False)
    # field is named differently in submission
    test_df['parcelid'] = test_df['ParcelId']

    print("2016 Training Size:" + str(train6_df.shape))
    print("2016 Property Size:" + str(properties_6.shape))

    print("2017 Training Size:" + str(train7_df.shape))
    print("2017 Property Size:" + str(properties_7.shape))

    properties_6 = changeGeoData(properties_6)
    properties_7 = changeGeoData(properties_7)

    properties_6 = setFeaturesSumAndVar(properties_6)
    properties_7 = setFeaturesSumAndVar(properties_7)

    for c in properties_6.columns.tolist():
        properties_6[c + "__nan__"] = properties_6[c].apply(lambda x:1 if pd.isnull(x) else 0)

    for c in properties_7.columns.tolist():
        properties_7[c + "__nan__"] = properties_7[c].apply(lambda x:1 if pd.isnull(x) else 0)


    numberOfNullCols6 = properties_6.isnull().sum(axis=1)
    numberOfNullCols7 = properties_7.isnull().sum(axis=1)

    properties_6 = changeObjects(properties_6)
    properties_7 = changeObjects(properties_7)

    properties_6 = changeDataTypes(properties_6)
    properties_7 = changeDataTypes(properties_7)

    d_median6 = properties_6.median(axis=0)
    d_median7 = properties_7.median(axis=0)

    properties_6["znull"] = numberOfNullCols6
    properties_7["znull"] = numberOfNullCols7

    train6_df = add_date_features(train6_df)
    train6_df = train6_df.merge(properties_6, how='left', on='parcelid')

    train7_df = add_date_features(train7_df)
    train7_df = train7_df.merge(properties_7, how='left', on='parcelid')

    test6_df = test_df.merge(properties_6, how='left', on='parcelid')
    test7_df = test_df.merge(properties_7, how='left', on='parcelid')

    print("Train: ", train6_df.shape)
    print("Test: ", test6_df.shape)
    ...
    exclude_missing6 = []
    num_rows = train6_df.shape[0]
    for c in train6_df.columns:
        num_missing = train6_df[c].isnull().sum()
        if num_missing == 0:
            continue
        missing_frac = num_missing / float(num_rows)
        if missing_frac > missing_perc_thresh:
            exclude_missing6.append(c)
    print("2016 We exclude: %s" % exclude_missing6)
    ...
    exclude_unique6 = []
    for c in train6_df.columns:
        num_uniques = len(train6_df[c].unique())
        if train6_df[c].isnull().sum() != 0:
            num_uniques -= 1
        if num_uniques == 1 and not "__nan__" in c:
            exclude_unique6.append(c)
    print("** 2016 We exclude: %s" % exclude_unique6)
    ...
    print("2016 We use these for training: %s" % train6_features)
    ...
    print("** 2016 Cat features are: %s" % [train6_features[ind] for ind in cat_feature_inds6])
    ...
    train6_df = train6_df.fillna(d_median6)
    test6_df = test6_df.fillna(d_median6)

    train7_df = train7_df.fillna(d_median7)
    test7_df = test7_df.fillna(d_median7)

    X_train6 = train6_df[train6_features]
    y_train6 = train6_df.logerror
    ...
    dates2016 = ["2016-10-01", "2016-11-01", "2016-12-01"]
    dates2017 = ["2017-10-01", "2017-11-01", "2017-12-01"]

    y_pred2016 = np.zeros(( test6_df.shape[0],3 ))
    for m in range(3):

        print("2016 date", dates2016[m])
        test6_df = test6_df.assign( transactiondate = dates2016[m])

        test6_df = add_date_features(test6_df)
        X_test6 = test6_df[train6_features]
        ...
        for i in tqdm(range(num_ensembles)):
            ...
            #3    iterations=200, learning_rate=0.03,
            ...
            model.fit(
                X_train6, y_train6,
                cat_features=cat_feature_inds6)
            y_pred6 += model.predict(X_test6)
        y_pred6 /= num_ensembles
        y_pred2016[:,m] = y_pred6

    df6 = pd.DataFrame(y_pred2016,columns=["201610","201611","201612"])
    df6.to_csv("output/y_pred2016_nan_.csv",index=False)
    ...
    for label in submission.columns.tolist()[1:4]:
        print("Predicting for: %s ... " % (label))
        # TODO(you): predict for every `test_date` 6
        submission[label] = df7[label]

    for label in submission.columns.tolist()[4:]:
        print("Predicting for: %s ... " % (label))
        # TODO(you): predict for every `test_date` 7
        submission[label] = df7[label]

    from datetime import datetime
    cdate = datetime.now().strftime("%Y%m%d%H%M%S")
    submission.to_csv(
        'output/sub_catboost_nan_{}.csv.gz'.format(cdate),
        float_format='%.4f',
