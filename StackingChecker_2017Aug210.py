...
def xgbModel_Valuation():
    ...
    xgb_select_features(m,X,y,X_test)

def HoldoutData_split(X,y,random_state=0):

    #
    #   1. Holdout model
    #
    HoldOutCls = HoldOutClass(X.shape[0],0.2,random_state)
    for h in HoldOutCls:
        (train_index,test_index) = h

    # 20% of total data is HoldOut
    print("20 percent --> HoldOut data ")
    X_holdout = X.loc[test_index,:]
    y_holdout = y[test_index]
    print("     HoldOut Datashape .." ,X_holdout.shape, y_holdout.shape)

    # 80% is training data ......
    print("80 percent --> training data ")
    X_train = X.loc[train_index,:]
    y_train = y[train_index]
    print("     Training Datashape ..",X_train.shape, y_train.shape)
