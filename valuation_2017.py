...
def valuation_2017(train_df, prop_df, propCounterCls):
    ...
    for dk, tdate in enumerate(test_dates):
        ...
        for fips_code in my_fips_codes:
            ...
            print(" submission file size to make .. ",submission_df.loc[ fips_mask_test.values, test_columns[dk] ].shape)


    print( "\nWriting results to disk ..." )
    submission_df.to_csv('output/sub_2017_{}.csv'.format(datetime.now().strftime('%Y%m%d_%H%M%S')), index=False)
    print( "\nFinished ...")
